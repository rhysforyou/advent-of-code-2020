...
fn main() {
    ...
    let (a, b, c) = three_expenses_with_sum(2020, &expenses)
        .expect("Couldn't find expenses that summed to 2020");
    let total = a * b * c;
...
fn two_expenses_with_sum(expected_sum: i32, expenses: &[i32]) -> Option<(i32, i32)> {
...
fn three_expenses_with_sum(expected_sum: i32, expenses: &[i32]) -> Option<(i32, i32, i32)> {
    for (index, expense) in expenses.iter().enumerate() {
        if index == expenses.len() - 2 {
            return None;
        }
        match two_expenses_with_sum(expected_sum - expense, &expenses[index + 1..expenses.len()]) {
            Some((a, b)) => return Some((*expense, a, b)),
            None => continue,
        }
    }
